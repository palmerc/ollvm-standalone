...
void addBogusFlow(BasicBlock *basicBlock, Function &F) {
  ...
  //                  block
  // We do this way, so we don't have to adjust all the phi nodes, metadatas and
  // so on for the first block. We have to let the phi nodes in the first part,
  // because they actually are updated in the second part according to them.
...
bool doF(Module &M) {
  ...
  // better way to obfuscate the predicates would be welcome. In the meantime we
  // will erase the name of the basic blocks, the instructions and the
  // functions.
  ...
  // Only for debug
  DEBUG_WITH_TYPE("cfg", errs() << "bcf: End of the pass, here are the graphs "
                                   "after doFinalization\n");
  for (Module::iterator mi = M.begin(), me = M.end(); mi != me; ++mi) {
...
bool BogusControlFlowLegacy::runOnFunction(llvm::Function &F) {
...
bool BogusControlFlow::runOnFunction(Function &F) {
  ...
  // If fla annotations
  return doF(*F.getParent());
}

PassPluginLibraryInfo getBogusControlFlowPluginInfo() {
  return {LLVM_PLUGIN_API_VERSION, "bcf", "v1.0", [](PassBuilder &PB) {
...
char BogusControlFlowLegacy::ID = 0;
RegisterPass<BogusControlFlowLegacy>
    BogusControlFlowX(/*PassArg=*/"legacy-bcf",
                      /*Name=*/"BogusControlFlowLegacy",
                      /*CFGOnly=*/false,
                      /*is_analysis=*/false);

llvm::FunctionPass *createBogus() { return new BogusControlFlowLegacy(); }
